...
- Map Figma design tokens to the project's token system (per the constitution's Design Token
  Management principle — discover whether it uses CSS variables, theme objects, or another approach).
...
- Ensure all interactive elements have accessible labels (per constitution's Accessibility Standards).
...
- NEVER mix server state and client state libraries (per constitution's State Management Conventions).
